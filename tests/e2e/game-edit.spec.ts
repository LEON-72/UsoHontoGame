...
test.describe('Game Editing', () => {
  ...
  test('should show edit form when game is in preparation status', async ({ page }) => {
    ...
    await expect(page.locator('button[type="submit"]')).toContainText('設定を更新');
  });
  ...
  test('should validate player limit is within range', async ({ page }) => {
    ...
    await expect(page.locator('text=参加人数は1〜100人の範囲で指定してください')).toBeVisible();
  });

  test('should prevent reducing limit below current players', async ({ page }) => {
  ...
  test('should have cancel button linking back to detail view', async ({ page }) => {
    // Given: On game detail page
    await page.goto('/games');
  ...
  test('should show error for non-existent game', async ({ page }) => {
    ...
    await expect(
      page
        .locator('text=ゲームが見つかりません')
        .or(page.locator('text=ゲームの読み込みに失敗しました'))
    ).toBeVisible();
  });
  ...
  test('should update timestamp after editing', async ({ page }) => {
    ...
    const _initialTimeText = await page.locator("dt:has-text('更新日時') + dd").textContent();
    ...
    const newTimeText = await page.locator("dt:has-text('更新日時') + dd").textContent();
