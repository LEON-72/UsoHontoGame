...
test.describe('Answer Submission - Participant Journey', () => {
	...
	test('US1: Participant can select and submit answers for active game', async ({ page }) => {
		...
		await expect(page.getByText(/各出題者のエピソードから嘘だと思うものを1つ選んでください/i)).toBeVisible();
		...
		const episodeButtons = page.getByRole('button', { pressed: false }).filter({ hasText: /エピソード/ });
	...
	test('Visual selection feedback when episode is clicked', async ({ page }) => {
		...
		const firstEpisodeButton = page.getByRole('button', { pressed: false }).filter({ hasText: /エピソード/ }).first();
	...
	test('Reset button clears all selections', async ({ page }) => {
		...
		// Select an episode
		const episodeButton = page.getByRole('button', { pressed: false }).filter({ hasText: /エピソード/ }).first();
	...
	test('FR-013: Resubmission updates existing answer (upsert)', async ({ page }) => {
		...
		if ((await episodeButtons.count()) > 0) {
			...
			if (await submitButton.isEnabled()) {
				...
				const unselectedButton = page.getByRole('button', { pressed: false }).filter({ hasText: /エピソード/ }).first();
...
test.describe('Answer Submission - Edge Cases', () => {
	...
	test('FR-009: Reject access when participant limit reached', async ({ page, context }) => {
		...
		test.skip(true, 'Requires programmatic game creation with participant limit and multiple sessions');
	});
});
