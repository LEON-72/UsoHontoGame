...
export function PresenterForm({ gameId, onPresenterAdded }: PresenterFormProps) {
  ...
  return (
    <div className="rounded-lg border border-gray-200 bg-white p-6">
      <h2 className="mb-4 text-lg font-semibold text-gray-900">プレゼンターを追加</h2>

      {isSuccess && (
        <div className="mb-4 rounded-md border border-green-200 bg-green-50 p-4" role="alert">
          <p className="text-green-800">プレゼンターを追加しました！</p>
        </div>
      )}
      ...
      <div className="mt-4 text-sm text-gray-600">
        <p>プレゼンターを追加後、3つのエピソード（2つのホント、1つのウソ）を登録してください。</p>
      </div>
    </div>
  );
}
