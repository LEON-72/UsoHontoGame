...
import { CompletePresenterSchema, ReadyToAcceptSchema } from '@/server/domain/schemas/validators';
...
describe('CompletePresenterSchema - exactly 3 episodes AND exactly 1 lie', () => {
  ...
  it('should reject presenter with 0 episodes', () => {
    expect(() => CompletePresenterSchema.parse({ episodes: [] })).toThrow(
      'エピソードは3つ登録する必要があります'
    );
  });

  it('should reject presenter with 1 episode', () => {
    expect(() => CompletePresenterSchema.parse({ episodes: [{ isLie: true }] })).toThrow(
      'エピソードは3つ登録する必要があります'
    );
  });
  ...
  it('should reject presenter with 4 episodes', () => {
    expect(() =>
      CompletePresenterSchema.parse({
        episodes: [{ isLie: false }, { isLie: true }, { isLie: false }, { isLie: false }],
      })
    ).toThrow('エピソードは3つ登録する必要があります');
  });
  ...
  it('should reject 3 episodes with 0 lies (all false)', () => {
    expect(() =>
      CompletePresenterSchema.parse({
        episodes: [{ isLie: false }, { isLie: false }, { isLie: false }],
      })
...
describe('ReadyToAcceptSchema - at least 1 complete presenter', () => {
  ...
  it('should reject game with 0 presenters', () => {
    expect(() => ReadyToAcceptSchema.parse({ presenters: [] })).toThrow('出題者が1人以上必要です');
  });
  ...
  it('should reject game with incomplete presenter (2 episodes)', () => {
    ...
    expect(() => ReadyToAcceptSchema.parse({ presenters: [incompletePresenter] })).toThrow(
      'エピソードは3つ登録する必要があります'
    );
  });
  ...
  it('should reject game with incomplete presenter (0 lies)', () => {
    ...
    expect(() => ReadyToAcceptSchema.parse({ presenters: [presenterWithoutLie] })).toThrow(
      'ウソのエピソードを1つ選択してください'
    );
  });
