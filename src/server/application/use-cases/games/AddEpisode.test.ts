...
describe('AddEpisode Use Case', () => {
  ...
  describe('Success Cases', () => {
    ...
    it('should allow adding up to 3 episodes', async () => {
      ...
      const episodes = await repository.findEpisodesByPresenterId(testPresenterId);
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle Japanese characters in episode text', async () => {
      // Given
      const japaneseText = 'これは私のエピソードです。今日は良い天気でした。';
  ...
  describe('Timestamp', () => {
    it('should set createdAt to current time', async () => {
      ...
      expect(result.episode.createdAt.getTime()).toBeGreaterThanOrEqual(beforeTime.getTime());
      expect(result.episode.createdAt.getTime()).toBeLessThanOrEqual(afterTime.getTime());
    });
  });
});
