...
describe('AddPresenter Use Case', () => {
  ...
  describe('Initial State', () => {
    ...
    it('should set createdAt to current time', async () => {
      ...
      expect(result.presenter.createdAt.getTime()).toBeGreaterThanOrEqual(beforeTime.getTime());
      expect(result.presenter.createdAt.getTime()).toBeLessThanOrEqual(afterTime.getTime());
    });
  });
});
